...
观察点设置
  ...
  可视域计算（calculateVisibility）
       │
       ├─ 生成 360 方向 × 自适应步数（≥10，采样间距 ≤50m）的球面采样点
...
## 问题 9：径向采样步长固定，大半径下漏判遮挡

**现象**：`STEPS_PER_RAY = 10` 固定，半径 5000m 时相邻采样点间距 500m，窄山脊/建筑落在两个采样点之间会被跳过，误判为可见。

**修复**：步长随半径自适应，`steps = max(MIN_STEPS, ceil(r / MAX_STEP))`，`MAX_STEP = 50m`、`MIN_STEPS = 10`。半径越大步数越多，采样间距恒定 ≤ 50m。

## 问题 10：LOS 遮挡容差过严导致误判

**现象**：`terrainH > losH + 0.5` 的 0.5m 容差小于 SRTM 30m DEM 的高程噪声，平缓地表被误判为遮挡。

**修复**：容差放宽为常量 `LOS_TOLERANCE = 1.5` 米。

## 问题 11：可见面积统计公式错误（按方向数占比，系统性低估）

**现象**：`visibleCount / grid.length * πr²` 把每个方向二分为「全程可见 / 完全不可见」，忽略被遮挡方向在遮挡点之前仍可见的部分，低估可见面积。

**根因**：扇区面积与可见距离平方成正比（`0.5·Δθ·dᵢ²`），不能按方向数线性占比。

**修复**：改为扇区面积求和 `Σ 0.5·Δθ·dᵢ²`，`dᵢ` 为每个方向的可见距离（全程可见 `r`，被遮挡 `blockDist`）。
